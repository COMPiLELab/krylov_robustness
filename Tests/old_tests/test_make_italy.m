% Test the performances of the various greeedy schemes for the make problem on either benchamrk or transportation graphs (determined by datasets{1} or dataset{2} in the for loop)
clear all

addpath ../functions/
addpath ../'MIOBI Codes'/

load('../voltage_adjacencies_average.mat')
column_names = {'method', 'dataset', 'n', 'm', ...
                'searchspace', 'centrality_order', 'time', 'tr_variation', 'budget_size'};
...
it = 100; 
debug = 0;
budget_array = 10; % floor(linspace(10,100,10)); % budget 
Q_array = 1; %[50 100 150 200]; % tunes the size of the search space in krylovmiobi_adaptive
...
debug  = false;
		...
		for k = budget_array
		    ...
		    for co = 2:2 % loop on centrality orders
		        %% KRYLOV MIOBI
		        method = "krylov_miobi";
		        size_search_space_kmiobi = min(1000, (n^2 - nnz(A) - n)/2);

		        tic;
		        E = find_top_missing_edges(A,centrality,size_search_space_kmiobi,centrality_order{co});
		        [krylovedges, tr_variation, A_new] = krylov_miobi(A, k, E, tol * nrm, it, poles, debug,'make');
		        time_kmiobi= toc;
		       
		        Results_TAB = [Results_TAB; ...
		            {method, name,n,nnz(A)/2,size_search_space_kmiobi,...
		            string(centrality_order{co}),time_kmiobi,tr_variation/trexp,budget_size}];
		        ...
		        for Q = Q_array
		            ...
		            [edges, tr_variation, A_new] = krylov_miobi_adaptive(A, k, Q, centrality, centrality_order{co}, tol * nrm, it, poles, debug,'make');
		            time_kmiobi_adaptive = toc;
	 
		            Results_TAB = [Results_TAB; ...
		            {method, name,n,nnz(A)/2,size_search_space_kmiobi_adaptive,...
		            string(centrality_order{co}),time_kmiobi_adaptive,tr_variation/trexp,budget_size}];
		    ...
		    EE = find_top_missing_edges(Asmall,centrality(ind_top_nodes),k,'min');
		    HURISTICEdges = [ind_top_nodes(EE(:,1)) ind_top_nodes(EE(:,2))];
		    ...
		    filepath = sprintf('../Results/results_make_vs_all_italy_%s.csv', string(date));
