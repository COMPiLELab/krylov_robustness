	...
	for i =  3:length(data) %start from 3 as first two are '.' and '..'
		...
		for k = budget_array
		    ...
		    for co = 1:2 % loop on centrality orders
		        ...
		        [krylovedges, tr_variation, A_new] = krylov_miobi(A, k, E, tol * nrm, it, poles, debug, 'break', rescale);
		        time_kmiobi= toc;
		    ...
		    filepath = sprintf('../Results/results_break_vs_all_misc_%s.csv', string(date));
