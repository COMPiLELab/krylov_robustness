...
% The adjacency matrices are rescaled to have norm 1
clear all
...
for datasets_list = { datasets{1} } % Modify this line to loop over benchmark or transportation graphs
	...
	for i =  3:length(data) %start from 3 as first two are '.' and '..'
		...
		%%% To rescale the adjacency matrices we estimate norm of A
		rescale = normest(A);
		A = A / rescale;
		nrm = exp(1);
		...
		for k = budget_array
		    ...
		    for co = 1:2 % loop on centrality orders
		        ...
		        [krylovedges, tr_variation, A_new] = krylov_miobi(A, k, E, tol * nrm, it, poles, debug,'make', rescale);
		        time_kmiobi= toc;
		        ...
		        for Q = Q_array
		            ...
		            [edges, tr_variation, A_new] = krylov_miobi_adaptive(A, k, Q, centrality, centrality_order{co}, tol * nrm, it, poles, debug,'make', rescale);
		        ...
		        p = randperm(size(E,1));
		        RANDOMEdges = E(p(1:k),:);
		        [U, B] = edge2low_rank(RANDOMEdges, n); B = B / rescale;
			...
			[U, B] = edge2low_rank([t1, t2], n); B = B / rescale;
		    delta_trace = lanczos_robustness_update(A, full(U), B, tol * nrm);
			time_miobi_25 = toc;
		    ...
		    [U, B] = edge2low_rank(HURISTICEdges, n); B = B / rescale;
			delta_trace = lanczos_robustness_update(A, full(U), B, tol * nrm);
			time_heuristic = toc;
			Results_TAB = [Results_TAB; ...
		    ...
		    filepath = sprintf('../Results/results_make_vs_all_misc_%s.csv', string(date));
