% Test the performances of the various greeedy schemes for the weighted break problem on either benchamrk or transportation graphs (determined by datasets{1} or dataset{2} in the for loop)
...
column_names = {'method', 'dataset', 'n', 'm', ...
                'searchspace_krylovmiobi', 'centrality_order', 'time', 'tr_variation'};
...
Q_array = [100 150 200]; % tunes the size of the search space in krylovmiobi_adaptive
	  

debug = false;

for datasets_list = { datasets{1} } % Modify this line to loop over benchmark or transportation graphs
    data = datasets_list{:}; 
	for i =  3:length(data) %start from 3 as first two are '.' and '..'
		...
		for k = budget_array
		    ...
		    for co = 1:2 % loop on centrality orders
				...
				E = [E, zeros(size(E, 1), 1)];
				for h = 1:size(E, 1)
					E(h, 3) = A(E(h, 1), E(h, 2));
				end
		        [krylovedges, tr_variation, A_new] = krylov_miobi_weighted(A, k, E, tol * nrm, it, poles, debug, 'break');
		        time_kmiobi= toc;
		       
		        Results_TAB = [Results_TAB; ...
		            {method, name,n,nnz(A)/2,size_search_space_kmiobi,...
		            string(centrality_order{co}),time_kmiobi,tr_variation}];
		        ...
		        for Q = Q_array
		            ...
		            [edges, tr_variation, A_new] = krylov_miobi_adaptive_weighted(A, k, Q, centrality, centrality_order{co}, tol * nrm, it, poles, debug,'break');
		            time_kmiobi_adaptive = toc;
	 
		            Results_TAB = [Results_TAB; ...
		            {method, name,n,nnz(A)/2,size_search_space_kmiobi_adaptive,...
		            string(centrality_order{co}),time_kmiobi_adaptive,tr_variation}];
		        ...
		        p = randperm(size(E,1));
		        RANDOMEdges = E(p(1:k),:);
				E = [RANDOMEdges, zeros(size(RANDOMEdges, 1), 1)];
				for h = 1:size(E, 1)
					E(h, 3) = A(E(h, 1), E(h, 2));
				end
		    	[U, B] = edge2low_rank_weighted(E, n);
		        delta_trace = lanczos_robustness_update(A, full(U), B, tol * nrm);
		        time_random = toc;
		        Results_TAB = [Results_TAB; ...
		            {method, name,n,nnz(A)/2,size_search_space_kmiobi,...
		            string(centrality_order{co}),time_random,delta_trace}];
				if debug 
					fprintf('Random done\n')
				end
		    end
		    ...
		    tic;
			[~, ~, Aorg] = MIOBIBreakEdge2_weighted(A,k,num_eig_miobi);
			[t1, t2, ~] = find(spones(spones(A) - spones(Aorg)));
			E = [t1, t2, zeros(size(t1, 1), 1)];
			for h = 1:size(E, 1)
				E(h, 3) = A(E(h, 1), E(h, 2));
			end
			[U, B] = edge2low_rank_weighted(E, n);
		    ...
		    E = [HURISTICEdges, zeros(size(HURISTICEdges, 1), 1)];
			for h = 1:size(E, 1)
				E(h, 3) = A(E(h, 1), E(h, 2));
			end
		    [U, B] = edge2low_rank_weighted(E, n);
			...
			time_heuristic = toc;
			Results_TAB = [Results_TAB; ...
		        {method, name,n,nnz(A)/2,k,"mult",time_heuristic,delta_trace}];
			if debug 
			...
			time_heuristic = toc;
			Results_TAB = [Results_TAB; ...
		        {method, name,n,nnz(A)/2,k,"min",time_heuristic,delta_trace}];
			if debug 
				fprintf('heuristic min done\n')
			end
		    
		   
		    filepath = sprintf('../Results/results_break_weighted_vs_all_street_%s.csv', string(date));
...
%-----------------------------------------------------------
function [U, B] = edge2low_rank_weighted(E, n)
