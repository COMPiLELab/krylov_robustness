function [edges, rob_variation, A_new] = krylov_miobi_adaptive_weighted(A, k, Q, centrality, order, tol, it, poles, debug, miobi)
	...
	if strcmp(miobi,'make')
		for j = 1 : k 
			...
			if j == 1
				...
				top_edges = [top_edges, zeros(Q + k, 1)];
				for h = 1:Q + k
					top_edges(h, 3) = A(top_edges(h, 1), top_edges(h, 2));
				end
			else
				n = length(top_edges);
				ind = find(prod(top_edges(:, 1:2) == tmp_edges(:, 1:2), 2));		% remove the previously selected edge from the search space
				top_edges = top_edges([1 : ind - 1, ind + 1 : n], :); 
			end
			E = top_edges(1:Q, :);
		    [tmp_edges, tmp_rob_variation, A] = krylov_miobi_weighted(A, 1, E, tol, it, poles, debug, miobi);
