function [edges, rob, A_new] = krylov_miobi_weighted(A, k, E, tol, it, poles, debug, miobi)
...
% E     	(optional) subset of edges with weights, that can be removed/added, default E = all edges of A
%       	E has to be a (k x 3) array such that E(j, 1) >= E(j, 2) for all j, for exploiting symmetry
...
%-----------------OUTPUT---------------------------------------------------------------------------------
%
% edges 	(k x 3) array containing the edges with weights to be removed/added
...
%--------------------------------------------------------------------------------------------------------
	...
	if ~exist('E', 'var') || isempty(E)
		[tmp1, tmp2] = find(A);
		ind = find(tmp1 >= tmp2);
		E = [tmp1(ind), tmp2(ind), A(tmp1(ind), tmp2(ind))];
		clear tmp1 tmp2;
	else
		%E = [E; [E(:, 2), E(:, 1)]];
		%ind = find(E(:, 1) >= E(:, 2));
	...
	for j = 1:min(k, nE)
		...
		for h = 1:nE 	
			if E(h, 1) ~= E(h, 2)		% build the low-rank correction to be applied to the argument
				if strcmp(miobi, 'break')
					B = -[0 A(E(h, 1), E(h, 2)); A(E(h, 1), E(h, 2)) 0];
	...
	%------------END-DEBUG-------------------------
		...
		if strcmp(miobi, 'break')
			...
			A(chosen(1), chosen(2)) = chosen(3); 		% add edge to the network
			A(chosen(2), chosen(1)) = chosen(3);
		end
