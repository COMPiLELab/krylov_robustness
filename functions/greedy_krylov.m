function [edges, rob_variation, A_new] = greedy_krylov(A, k, Q, centrality, order, tol, it, poles, debug, miobi, rescale)
...
if ~issymmetric(A)
	error('GREEDY_KRYLOV:: Adjacency matrix should be symmetric');
end
if ~exist('tol', 'var')
	tol = 1e-12;
end
...
if strcmp(miobi, 'break') && nnz(A) < 2*k 
	error('GREEDY_KRYLOV:: edges to be removed are more than edges in the network')
end
...
if strcmp(miobi,'make')
    for j = 1 : k 
		...
		if j == 1
        	top_edges = find_top_missing_edges(A, centrality, Q + k, order); 
		else
			n = length(top_edges);
