function [Um, Xm, Vm, row, col, iter] = multiple_frechet_eval(A, omega, f, tol, it, poles, debug)
...
for j = 1:it
	...
	for h = I
		...
		if isscalar(poles) && poles == inf			
			...
			error('MULTIPLE_FRECHET_EVAL::Unsupported rational Krylov yet')
		end
			temp = Um{row(h)}' * U;
			Uaux(row(h)) = temp(1); 
		else
	...
	for h = J
		if j == 1
			...
			if isscalar(poles) && poles == inf			
				[Vm{col(h)}, KB{col(h)}, HB{col(h)}, param_B{col(h)}] = arnoldi_krylov(AT,  V);
			else
			error('MULTIPLE_FRECHET_EVAL::Unsupported rational Krylov yet')
			end
			temp = Vm{col(h)}' * V;
			...
			if isscalar(poles) && poles == inf			
				[Vm{col(h)}, KB{col(h)}, HB{col(h)}, param_B{col(h)}] = arnoldi_krylov(Vm{col(h)}, KB{col(h)}, HB{col(h)}, param_B{col(h)});
			else
			error('MULTIPLE_FRECHET_EVAL::Unsupported rational Krylov yet')
			end
...
iter = j;
if iter == it
	warning('MULTIPLE_FRECHET_EVAL:: Reached maximum number of iterations')
end
for j = 1:nI
	Um{j} = Um{j}(:, 1:end - rk);
end
