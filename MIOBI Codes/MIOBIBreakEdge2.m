function [robScoreNoUpdate, robScoreUpdate50, Aorg] = MIOBIBreakEdge2(fname,k,topT)
...
for j = 1:k
    ...
    Aorg(remove(1), remove(2)) = 1-ep;
    Aorg(remove(2), remove(1)) = 1-ep;
    
    deltaA = sparse(N,N);
    %update top t eigenvalues of A equation (4)
    deltaA(remove(1), remove(2)) = -ep;
    deltaA(remove(2), remove(1)) = -ep;
...
for j = 1:k
    ...
    A(remove(1), remove(2)) = 1-ep;
    A(remove(2), remove(1)) = 1-ep;
